...
/* 🔴 CRUCIAL NOTE:
 ...
 This problem can be solved more efficiently using vectors (std::vector),
 which will be introduced in the upcoming C++ Level II course.

 However, the current solution intentionally avoids vectors and is considered
 a *primitive / foundational approach*.

 👉 Purpose:
 To strengthen core programming skills through basic constructs.

 The problem is solved using two foundational approaches:

 1️⃣ Using separate variables:
 → Example: number1, number2, number3

 2️⃣ Using arrays (Fixed-length data structures):
 

 ⚠️ Important Insight:
 This is NOT about writing the most optimal or modern solution.
 This is about building strong fundamentals before abstraction layers (like vectors).
...
const int NumberOfArrayElements = 3;   // found out later, that it has to be global AND has to be declared as a constant variable

int counter = 0;  // it seems that count is a reserved keyword ??
...
float ReadNumber(const string& message)
{
    ...
    counter++;                   // was added at the VERY END when the program worked fine.
    string FinalMessage = message + to_string(counter) + " : ";  // was added at the VERY END when the program worked fine.
...
bool ValidateArrayElementWithRestriction(const float &number) // pass by constant reference.  
{
    return (number != 0);
}

void ReadArrayWithRestriction(const string &message, float numbers[NumberOfArrayElements]) // pass by constant reference. // array passed by reference automatically (including & will render exception).
{                                                                                                 // range ALWAYS passed by value.  
    for(int i = 0; i < NumberOfArrayElements; i++) {
    do
    {
        numbers[i] = ReadNumber(message);  // 'numbers[i] to be IMMEDIATELY evaluated against the range, in the while condition.

    } while(!ValidateArrayElementWithRestriction(numbers[i])); // 👈👈 the argument is written so that, it evaluates to TRUE when the condition is INVALID

    }

    //NOTE: the above block is to be composed line by line
    // & at the end, implement the functions of:   ReadNumber  &   ValidateArrayElementWithRestriction()
}

float CalculateSumOfNumbers(float numbers[NumberOfArrayElements]) // automatically passed by reference whether you like it or not (arrays will not be copied where it's stored in another memory space that has a separate scope
                                                                  // and this is comrehendable as an array is memory space consuming.
{
...
void PrintSumOfNumbers(float numbers[NumberOfArrayElements]) // pass by reference (the reference part is BY DEFAULT without including the & sign as doing so renders exception)
{                                        // you can NOT add constant in this specific argument; doing so won^t enable you to add the numbers up.
...
int main()
{
    ...
    // initializing the above array variable, via reading user input (Using a procedure) since an array has more than one element:

    ReadArrayWithRestriction(" Enter number ", numbers); // the array 'numbers' is passed along with its address in memory AUTOMATICALLY (so, its implementation will NOT have the & sign).
    // the array variable 'numbers' has been initialized.

    PrintSumOfNumbers(numbers); // the argument that's the array variable is sent along with it's address in memory (hence- in the implementation, it's automatically passed by reference without the & sign)
    // the nested functions line is designed so that, the printing procedure contains the function CalculateSumOfNumbers()
...
//
//unsigned short int counter = 0; // written when it turned out that it was needed.
//
...
//    counter++;
//    string FinalMessage = "";
//    FinalMessage = message + to_string(counter) + ": ";                                   // message = message + to_string(counter); // ❌ as message is const
//
//    cout << FinalMessage;
//    cin >> number;
//    cout << endl;
...
//}
//
//bool ValidateUserInput(const float& number, int min, int max)  // pass by constant reference  // Ranges ALWAYS passed by value.
//{
...
//
//
//void ReadUserInputs(const string &message, float &Number1, float &Number2, float &Number3, int min, int max) 
//{
//    do
//    {
//        Number1 = ReadNumber(message); // 'Number1' is to be IMMEDIATELY validated against the range, in the while condition.
//
//    } while (!ValidateUserInput(Number1, min, max));   // the argument is written so that it evaluates to TRUE when the condition is invalid.
//
//    do
//    {
//        Number2 = ReadNumber(message);  // 'number2' is to be IMMEDIATELY validated against the range, in the while condition.
//
//    } while (!ValidateUserInput(Number2, min, max)); // the argument is wirtten so that it evaluates to TRUE when the condition is invalid.
//
//    do
//    {
//        Number3 = ReadNumber(message);  // 'Number3' is to be IMMEDIATELY validated against the range, in the while condition
//
//    } while (!ValidateUserInput(Number3, min, max));
//
//    //NOTE: The above block is to be composed line by line
//    // & at the end, implement the functions of: ReadNumber()  &  ValidateUserInput()
//}
//
//float CalculateTheSum(float Number1, float Number2, float Number3)   // pass by value
//{
//    return (Number1 + Number2 + Number3);
//}
//
//void PrintSumOfNumbers(const float& Number1, const float& Number2, const float& Number3) // pass by const. ref.
//{
//    cout << "The sum is " << CalculateTheSum(Number1, Number2, Number3) << endl;
//}
//
//
//int main()
...
//    // initializing the above variables via reading user input (using a procedure- no one value is returned):
// 
//    ReadUserInputs( "Enter number ", Number1, Number2, Number3, min, max); // ranges included as arguments, of the reading procedure, as their values are NOT intrinsic. 
//    // Number1, Number2, & Number3 are NOW INITIALIZED.
//
//    PrintSumOfNumbers(Number1, Number2, Number3);// composed so that the printing procedure's implementation contains CalculateSumOfNumbers().
//
//    
//}
//
