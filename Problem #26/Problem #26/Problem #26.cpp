...
bool ValidateNRange(const int& n)    // pass by constant ref. since n is only used for comparison against the ranges.
{
	return (n > 0);
}
...
int ReadN(const string& message)    // pass by constant reference.
{
	...
	// ReadIntegerNumber()   &  ValidateNRange()
}


void PrintNumbersFrom1ToN_UsingForLoops(const int& n)
{   
...
void PrintNumbersFrom1ToN_UsingWhileLoop(const int& n)
{
	cout << "Solving it using a While-Loop: \n";

	int i = 1;       // the 1st line to write.
	while (i <= n)    // the 2nd line to write.
	{
		cout << i << endl;   // THE LAST LINE to compose

		i++;   // the 3rd line to write.
	}
...
void PrintNumbersFrom1ToN_UsingDoWhileLoop(const int& n)
{
...
int main()
{
	int n = ReadN("Enter a number: ");    // Range(s) NOT included as their value is Not intrinsic.

	PrintNumbersFrom1ToN_UsingForLoops(n);   
