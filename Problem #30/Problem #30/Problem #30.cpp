...
bool ValidatePositiveInteger(const int& number)    // pass by constant reference since number is oinly used for comparison against range.
{
	return (number > 0);
}
...
long long CalculateFactorialOfN_UsingForLoops(int number)   // pass by value // the return type is NOT int. as factorial results GROW VERY LARGE VERY QUICKLy (Ex 1! is so huge that it surpasses the range for int)
{
	long long multiply = 1;   // routine   // what's returned.
...
///////////////////////////////////////////////////////////////////
// Using, instead, a while loop:

long long CalculateFactorialOfN_UsingWhileLoop(int number)   // pass by value.
{
	long long multiply = 1;  // rountine   // what's returned.

	int i = number;    // 1st line to write
	while (i >= 1)  // 2nd line to write
	{
		multiply = multiply * i;

		i--;   // third line to write.
	}
...
// using, instead, a do..while statement
long long CalculateFactorialOfN_UsingDoWhileLoop(int number)   // pass by value
{
	long long multiply = 1;   // routine   // what's returned

	int i = number;   // 1st step to compose
	do
	{
		multiply = multiply * i;

		i--;    // 2nd step to compose

	} while (i >= 1);  //3rd step to compose
...
void PrintFactorialOfN(const int& number)    // pass by constant reference
{
	cout << "The factorial of " << number << " [using For-Loops] is " << CalculateFactorialOfN_UsingForLoops(number) << endl;
	cout << "The factorial of " << number << " [using While-Loop] is " << CalculateFactorialOfN_UsingWhileLoop(number) << endl;
	cout << "The factorial of " << number << " [using Do-While-Loop] is " << CalculateFactorialOfN_UsingDoWhileLoop(number) << endl;
}
...
int main()
{
	...
	PrintFactorialOfN(ReadPositiveInteger("Enter a positive integer: ")); // range NOT included as its value is NOT intrinsic.  // The argument HAS to be a function.
	// this printing procedure contains the function CalculateFactorialOfN() as
	// this is a standard coding practice.
	// Additionally, and as a result, the printing procedure will have access to the positive number that the user entered.
...
//////////////////////////////////////////////////////////
//// But if I want the error message to be more deliberate as in "Invalid. Please enter a positive integer" 
//// instead of keeping on asking the same question "Please enter a positive integer" until the user enters a positive number, then consider the 
//// below solution:    
//// 1- Using While loop (The cleanest):
...
long long CalculateFactorialOfN_UsingForLoops(int number)   // pass by value // the return type is NOT int. as factorial results GROW VERY LARGE VERY QUICKLy (Ex 12! is so huge that it surpasses the range for int)
...
long long CalculateFactorialOfN_UsingWhileLoop(int number)   // pass by value.// the return type is NOT int. as factorial results GROW VERY LARGE VERY QUICKLy (Ex 12! is so huge that it surpasses the range for int)
{
	long long multiply = 1;  // rountine   // what's returned.
...
// using, instead, a do..while statement
long long CalculateFactorialOfN_UsingDoWhileLoop(int number)   // pass by value // the return type is NOT int. as factorial results GROW VERY LARGE VERY QUICKLy (Ex 12! is so huge that it surpasses the range for int)
{
	long long multiply = 1;   // routine   // what's returned
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 2- Using a counter variable inside the do statement (not the best solution as counter is created/ used):
...
long long CalculateFactorialOfN_UsingForLoops(int number)   // pass by value
{
	long long multiply = 1;   // routine   // what's returned.
