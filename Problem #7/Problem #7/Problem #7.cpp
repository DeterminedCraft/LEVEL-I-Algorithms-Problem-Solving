...
bool ValidateFloatWithRestriction(const float& number)   // pass by constant reference.
{
	return (number != 0);
}
...
float ReadFloatWithRestriction(const string& message) // passing by constant reference
{
	float number; // what's returned
	do
	{
	...
	} while (!ValidateFloatWithRestriction(number)); // the argument is written so that it evaluates to TRUE when the condition is invalid (hence the ! sign)

	return number;

	//NOTE: The above block of code is to be written line by line
	// and at the end, implement the function of:
	// ReadNumber()  &  ValidateFloatWithRestriction()
}

float CalculateHalfTheNumber(float number) // pass by value
{
	return number / 2; // this will NOT be an integer division as number is declared as float. so if user enters 3, then it's stored as 3.0. leading to 3.0 / 2 = can NOT be integer division.
}

void PrintHalfTheNumber(const float &number)   // pass by constant reference
{
...
int main()
{
	...
	PrintHalfTheNumber(ReadFloatWithRestriction(" Enter a number: ")); // the argument HAS to be a function; as procedure returns void (can never be an argument as it renders an exception)
	                                                // no range argument inside the reading function as it's value is intrinsic (the restriction is 0 in this case)

	// the nested functions line above is written as is, as CalculateHalfTheNumber() is embedded within the printing procedure.
	// this is also HAPPENS TO BE CONDUCIVE, to the printing procedure having access to the original number the user enter, so that it could be referenced 
	// by the pringing procedure (ex: cout << "half of " << number << " is " << CalculateHalfTheNumber() << endl;).

	// the same was as saying: 
...
////////////////////////////////////////// BELOW: Some older first attempl solutions //////////////////////////////////////////////////////
