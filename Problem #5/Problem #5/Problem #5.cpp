...
// that's either going to be a normal variable (ONE value) or an array variable (MORE than one variable)
//
...
// - If it's going to be initialized via reading user input, then declare it inside a function of the structure user-defined data type
// that returns this variable. Since, it's the latter then IMMEDIATELY start implemneting this function NOW below:

bool ValidateSomeApplicantInfo_BooleanValue(const int& number) // passing by constant reference.
{
	return (number == 0 || number == 1);
}


bool ReadSomeApplicantInfo(const string &message) // pass by constant reference.
{
	...
	do
	{
		...
		cout << message;
		cin >> number; // 'number' is to be IMMEDIATELY evaluated against the range, in the while condition.
		cout << endl;

	} while (!ValidateSomeApplicantInfo_BooleanValue(number));

	return number; // before it's returned to the calling function, to evaluate to it, it's implicitly copnverted from int to bool.
}

unsigned short ReadApplicantInfoAge(const string &Message1) // pass by const. ref.
{
...
bool ValidateApplicantInfoAge(const unsigned short& age, unsigned short min, unsigned short max) // passing by constant reference // the range is for the input the user enters for age. it has NOTHING to do with the age requirement of 21 presented in the requirement.
{
	return (min <= age && age <= max);
}

stApplicantInfo ReadApplicantInfo(const string &Message1, const string &Message2, const string &Message3, unsigned short min, unsigned short max) // passing by constant reference. // ranges ALWAYS passed by value
{
	stApplicantInfo ApplicantInfo; // what's returned  // the next natural step of declaring a variable of this structure user-defined data type.
	                               // AS SOON As you see such line, ALWAYS envision ApplicantInfo having an access to all member variables defined inside the structure.
	do
	{
		ApplicantInfo.age = ReadApplicantInfoAge(Message1); // 'ApplicantInfo.age' to be IMMEDIATELY validated against the range, in the while condition.

	} while (!ValidateApplicantInfoAge(ApplicantInfo.age, min, max));


	ApplicantInfo.HasDriverLicense = ReadSomeApplicantInfo(Message2); // "ApplicantInfo.HasDriverLicense" is to be IMMEDIATELY validated against the range, in the while condition.

	ApplicantInfo.HasRecommendation = ReadSomeApplicantInfo(Message3); //  "ApplicantInfo.HasRecommendation" is to be IMMEDIATELY validated against the range, in the while condition.


	return ApplicantInfo;
	// NOTE: The above lines of code are to be sequentially composed line by line
	// and at the end, implement the functions of: 
	// ReadApplicantInfoAge(), ValidateApplicantInfoAge(), ReadSomeApplicantInfo(), ValidateApplicantInfo_BooleanValue()

	/* NOTE THAT : 
	
	 1- ApplicantInfo.HasDriverLicense  & ApplicantInfo.HasRecommendation  SHARE ONE reading function because both are of the SAME DATA TYPE (int, float, double, etc)  (that is bool in this specific example/case)

	 2- 
	     ApplicantInfo.HasDriverLicense  &  ApplicantInfo.HasRecommendation 
		 share the same range validation function : ValidateSomeApplicantInfo_BooleanValue
		 
		 because
		 both are boolean STRICTLY SPEAKING. 
		 Where the input has to be either 1 or 0 and that's about it !.
	
	
    */
}

bool IsHired(stApplicantInfo ApplicantInfo)  // passing by value. // WHENEVER you see such line, ALWAYS envision the variable having access to all member variables inside the structure.
{
	return ((ApplicantInfo.age > 21 && ApplicantInfo.HasDriverLicense) || ApplicantInfo.HasRecommendation);
}

void PrintHiredOrNot(const stApplicantInfo& ApplicantInfo) // pass by constant reference
{
...
int main()
{
	// instead of creating the variables 'age', 'HasDriverLicense', 'HasRecommendation' in main()
	// I'll instead declare them as member variables inside a structure, since they are related amongst each other.

	unsigned short min = 16;
	unsigned short max = 130;

	PrintHiredOrNot(ReadApplicantInfo(" Enter age: ", " Do you have a Driver License (Enter 1 for yes  / Enter 0 for no) ?: ", " Do you have a recommendation (Enter 1 for yes  / Enter 0 for no) ?: ", min, max)); 
	// composed using one thought: IsHired() as an argument of an If statement inside the printing procedure.
   // 2nd thing was including the string messages.
